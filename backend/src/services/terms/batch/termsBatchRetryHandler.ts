 ...
 */
export interface TermsBatchRetryConfig {
  /**
   * 最大リトライ回数
   * @default 3
   */
   ...
   */
  onRetry?: (attempt: number, delayMs: number, result: TermsBatchResult) => void;
}
 ...
 * リトライ処理全体の結果を表す
 */
export interface TermsBatchRetryResult {
  /** 最終的な成功/失敗 */
  success: boolean;
 ...
 */
export class TermsBatchRetryHandler {
   ...
   * @param config - リトライ設定
   */
  constructor(config: TermsBatchRetryConfig = {}) {
   ...
   * @returns 設定オブジェクト
   */
  getConfig(): Required<Omit<TermsBatchRetryConfig, 'onRetry'>> {
    return {
   ...
   * 部分成功（一部の用語のみ成功）の場合はリトライしない
   *
   * @param executeFn - バッチ処理を実行する関数
   * @returns リトライ結果
   */
  async executeWithRetry(
    executeFn: () => Promise<TermsBatchResult>
  ): Promise<TermsBatchRetryResult> {
    const startTime = Date.now();
    let attemptCount = 0;
    let lastResult: TermsBatchResult | null = null;
    ...
    while (attemptCount <= this.maxRetries) {
      ...
      try {
        ...
        if (lastResult.success || lastResult.partialSuccess) {
          console.log(
            `[TermsBatchRetryHandler] Batch completed on attempt ${attemptCount}. ` +
        ...
        if (attemptCount <= this.maxRetries) {
          ...
          console.log(
            `[TermsBatchRetryHandler] Attempt ${attemptCount} failed. ` +
        ...
        if (attemptCount <= this.maxRetries) {
          ...
          console.log(
            `[TermsBatchRetryHandler] Attempt ${attemptCount} threw exception: ` +
    ...
    console.log(
      `[TermsBatchRetryHandler] Retry handler completed. ` +
   ...
   * @returns バッチ結果
   */
  private createErrorResult(error: unknown): TermsBatchResult {
    return {
      success: false,
   ...
   * @returns 空のバッチ結果
   */
  private createEmptyResult(): TermsBatchResult {
    return {
      success: false,
      partialSuccess: false,
      firestoreSaved: false,
