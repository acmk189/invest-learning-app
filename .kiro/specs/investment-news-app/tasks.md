...
- [ ] 3.1 Claude APIクライアント導入と初期化 (~200行)
...
- [ ] 6.3 NewsAPIレスポンスパース機能 (~150行)
  - APIレスポンスから記事タイトルを抽出する機能を実装する
  - 記事説明・本文を抽出する機能を実装する
  - パース結果の型定義を作成する
  - _Requirements: 1.2_

- [ ] 6.4 NewsAPI無料枠制御機能 (~150行)
  - 1日1回のみ実行する制御を実装する
  - 無料枠（100リクエスト/日）の使用状況を記録する機能を実装する
  - 枠超過時の警告ログを実装する
  - _Requirements: 10.2_

- [ ] 6.5 NewsAPIエラーハンドリング (~200行)
...
- [ ] 12.1 用語バッチリトライロジック (~200行)
  - バッチ処理失敗時に最大3回リトライする機能を実装する
  - リトライ間隔を指数バックオフで設定する
  - リトライ回数のログを実装する
  - _Requirements: 8.3_

- [ ] 12.2 用語ステップ別エラーログ記録 (~200行)
  - 用語生成ステップでのエラーログを実装する
  - 重複チェックステップでのエラーログを実装する
  - Firestore保存ステップでのエラーログを実装する
  - _Requirements: 8.5, 11.3_

- [ ] 12.3 用語部分成功検出・ハンドリング (~200行)
  - 2つの用語のみ生成成功の検出機能を実装する
  - 1つの用語のみ生成成功の検出機能を実装する
  - 部分成功時の保存・通知機能を実装する
  - _Requirements: 8.2_

- [ ] 12.4 用語最終失敗時詳細ログ保存 (~150行)
  - 最終的な失敗時にFirestore `error_logs`に詳細ログを保存する機能を実装する
  - エラー詳細の構造を定義する
  - _Requirements: 8.5_
...
- [ ] 14.1 (P) Firestore接続初期化 (~200行)
  - React Nativeアプリ用のFirestore接続を初期化する
  - 接続設定の型定義を作成する
  - 接続テストを実装する
  - _Requirements: 12.3_

- [ ] 14.2 (P) 日付ベースクエリ実装 (~200行)
  - 今日の日付でニュースを取得するクエリを実装する
  - 今日の日付で用語を取得するクエリを実装する
  - クエリ結果の型定義を作成する
  - _Requirements: 2.1, 5.1_

- [ ] 14.3 (P) オフライン永続化設定 (~150行)
  - オフライン永続化を有効化する
  - キャッシュ動作を検証する
  - キャッシュサイズの設定を実装する
  - _Requirements: 2.5_

- [ ] 14.4 (P) Firestoreエラーハンドリング (~200行)
...
- [ ] 15.1 (P) Expo Routerタブ設定 (~200行)
  - Expo Routerでファイルベースタブナビゲーションを設定する
  - 「ニュース」「用語」の2タブを実装する
  - タブルーティングの設定を実装する
  - _Requirements: 6.1_

- [ ] 15.2 (P) タブアイコン・ラベル実装 (~150行)
  - タブアイコンをデザインする
  - タブラベルを実装する
  - iOS標準UIに準拠したスタイルを適用する
  - _Requirements: 6.1_

- [ ] 15.3 (P) タブ切り替え最適化 (~150行)
...
- **5.1〜5.5 → 10.1〜10.4**: 用語生成機能が必要
- **10.1〜10.4 → 11.1〜11.6**: 重複チェック機能が必要
...
4. **Phase 4: 用語バッチ（タスク10〜13）**
   - タスク 10.1〜10.4（用語重複チェック）
   - タスク 11.1〜11.6（用語バッチ処理）
